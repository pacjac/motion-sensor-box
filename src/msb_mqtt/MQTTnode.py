...
class MQTTnode:
    def __init__(self, config_override={}, debug=False):
        ...
        if self.debug:
            print(f"Subscribing to topics: {self.config.topics}")
        self.subscriber = Subscriber(
            connect_to=self.config.xpub_socketstring, topics=self.config.topics
        )
        self.connect_mqtt()
    ...
    def wait_for_input_and_publish(self):
        ...
        while True:
            # This is blocking
            try:
                # ZQM transports a pickled dict
                # with topic as key and value as value
                ...
                # New, using lists
                for topic, value in zip(self.topics, data):
                    self.publish(topic, value)

                # We comment this out for now,
                # but I would like to replace this with an extra function
    ...
    Message should look as follows (whitespace important!)
